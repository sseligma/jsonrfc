...
Class JsonRfc {
	...
	private static $rfcResults = array();
	...
	public static function execute($params=null,$x509=null,$format=0) {
		...
		// If ther are errors, create a results array with the error flag set to true and add the errors to the errorMessage array
		if (count(JsonRfc::$errors)) {
		...
		// at this point rfcResults are populated with results from the rfc call or error messages
		// return the rfcResults as is or return a json encoded string based on the $format value
		if (JsonRfc::$format == JsonRfc::$FORMAT_JSON) {
			return json_encode(JsonRfc::$rfcResults);
		} 
		else {
			return JsonRfc::$rfcResults;
		}
